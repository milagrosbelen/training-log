...
import { createTraining, getTrainings } from "./utils/api"
...
function App() {
  ...
  // Cargar entrenamientos desde el backend al iniciar
  useEffect(() => {
    const loadTrainings = async () => {
      try {
        ...
        localStorage.setItem("workouts", JSON.stringify(trainings))
      } catch (error) {
        console.error("Error al cargar entrenamientos desde el backend:", error)
        ...
        if (savedWorkouts) {
          try {
            ...
            setToast({ 
              message: "No se pudo conectar al servidor. Mostrando datos locales.", 
              type: "error" 
            })
          } catch (parseError) {
            console.error("Error al parsear entrenamientos de localStorage:", parseError)
          }
        }
      }
    }
  ...
  // Guardar entrenamientos en localStorage cuando cambien (como backup)
  useEffect(() => {
  ...
  const handleSaveWorkout = async (workoutData) => {
    try {
      // 1. Crear el entrenamiento en el backend con POST /trainings
      await createTraining(workoutData)
      
      // 2. Obtener la lista completa actualizada con GET /trainings
      const updatedTrainings = await getTrainings()
      
      // 3. Actualizar el estado con los datos del backend
      setWorkouts(updatedTrainings)

      // 4. Mostrar mensaje de éxito
      const quote = getRandomQuote(successQuotes)
      setToast({ 
        message: `Entrenamiento guardado correctamente. ${quote}`, 
        type: "success" 
      })

      // 5. Limpiar workout copiado temporal
      setCopiedWorkout(null)
      ...
      setToast({ 
        message: `Error al guardar el entrenamiento: ${error.message}`, 
        type: "error" 
      })
    }
  }
